...
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False  # wyłącz opcję SQLALCHEMY_TRACK_MODIFICATIONS
...
def koszyk():
    ...
    for row in products:
        totalPrice += row[2]
    return render_template("koszyk.html", products = products, totalPrice=totalPrice, loggedIn=loggedIn, firstName=firstName, noOfItems=noOfItems)

@app.route("/removeFromkoszyk")
def removeFromkoszyk():
    if 'email' not in session:
        return redirect(url_for('loginForm'))
    email = session['email']
    productId = int(request.args.get('productId'))
